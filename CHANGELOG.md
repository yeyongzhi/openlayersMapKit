...
- 修复 Draw 一次完成派发两次 `drawend` 的问题。事件快照包含新完成的要素，程序化向绘制图层添加要素不再合成绘制完成事件。回调沿用 OpenLayers 的 source 写入前时序。

### Added

- 增加本地矢量数据的真实浏览器交互测试、消费者类型负向断言和独立 tarball 安装验收。
- 统一 CI 与本地完整验收，补齐 API 名称基线、文档链接、示例和发布规则检查。
- 增加 LF 换行规则、分支保护导入模板与默认不发布的 npm/Pages 演练流程。
- 明确 `dev` 直接开发、`dev → main` 通过 PR 的双分支管理策略。
