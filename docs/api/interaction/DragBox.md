...
```ts
import { DragBox } from 'openlayers-map-kit'
```

源码：`src/module/interaction/DragBox/index.ts`

## 构造

```ts
new DragBox(params?: (ManualOmit<Options, "onBoxEnd"> & { onBoxEnd?: DragBoxEndEventFunctionType | undefined; } & OMapInteractionCommonParamsType))
```
...
| 属性                                 | 说明                                                 |
| ------------------------------------ | ---------------------------------------------------- |
| `events: Event<OMapDragBoxEventMap>` | 收窄交互事件总线类型（构造器中以具体事件映射实例化） |
| `extent: Extent \| null`             | —                                                    |
...
| 方法                                                                                                                                                                            | 说明 |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| `on(type: "change" \| "error" \| "propertychange" \| "change:active" \| "boxcancel" \| "boxdrag" \| "boxend" \| "boxstart", callback: (e: OMapDragBoxEvent) => void): string`   | —    |
| `once(type: "change" \| "error" \| "propertychange" \| "change:active" \| "boxcancel" \| "boxdrag" \| "boxend" \| "boxstart", callback: (e: OMapDragBoxEvent) => void): string` | —    |
| `un(id: string): void`                                                                                                                                                          | —    |

## 继承成员

继承链：DragBox → Interaction
