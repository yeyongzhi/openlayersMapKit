...
```ts
new KeyboardZoom(params?: (OMapInteractionCommonParamsType & ManualOmit<Options, "">))
```
...
| 属性                                      | 说明                                                 |
| ----------------------------------------- | ---------------------------------------------------- |
| `events: Event<OMapKeyboardZoomEventMap>` | 收窄交互事件总线类型（构造器中以具体事件映射实例化） |

## 方法

| 方法                                                                                                                           | 说明 |
| ------------------------------------------------------------------------------------------------------------------------------ | ---- |
| `on(type: "change" \| "error" \| "propertychange" \| "change:active", callback: (e: OMapKeyboardZoomEvent) => void): string`   | —    |
| `once(type: "change" \| "error" \| "propertychange" \| "change:active", callback: (e: OMapKeyboardZoomEvent) => void): string` | —    |
| `un(id: string): void`                                                                                                         | —    |

## 继承成员

继承链：KeyboardZoom → Interaction
