...
```ts
import { DragPan } from 'openlayers-map-kit'
```

源码：`src/module/interaction/DragPan/index.ts`

## 构造

```ts
new DragPan(params?: (ManualOmit<Options, "onBoxEnd"> & OMapInteractionCommonParamsType))
```
...
| 属性                                 | 说明                                                 |
| ------------------------------------ | ---------------------------------------------------- |
| `events: Event<OMapDragPanEventMap>` | 收窄交互事件总线类型（构造器中以具体事件映射实例化） |

## 方法

| 方法                                                                                                                      | 说明 |
| ------------------------------------------------------------------------------------------------------------------------- | ---- |
| `on(type: "change" \| "error" \| "propertychange" \| "change:active", callback: (e: OMapDragPanEvent) => void): string`   | —    |
| `once(type: "change" \| "error" \| "propertychange" \| "change:active", callback: (e: OMapDragPanEvent) => void): string` | —    |
| `un(id: string): void`                                                                                                    | —    |

## 继承成员

继承链：DragPan → Interaction
