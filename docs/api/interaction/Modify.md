...
修改交互，对已有要素的几何进行拖拽与顶点编辑。

## 引入

```ts
import { Modify } from 'openlayers-map-kit'
```

源码：`src/module/interaction/Modify/index.ts`

## 构造

```ts
new Modify(params: OMapModifyParamsType)
```
...
| 属性                                | 说明                                                 |
| ----------------------------------- | ---------------------------------------------------- |
| `events: Event<OMapModifyEventMap>` | 收窄交互事件总线类型（构造器中以具体事件映射实例化） |
| `records: ModifyRecordItem[]`       | —                                                    |

## 方法

| 方法                                                                                                                                                     | 说明                                 |
| -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `cancel(): void`                                                                                                                                         | 取消当前全部修改，也就是回到初始状态 |
| `canInsertPoint(): boolean`                                                                                                                              | —                                    |
| `canRemovePoint(): boolean`                                                                                                                              | —                                    |
| `insertPoint(coordinates: OMapCoordinateType): boolean`                                                                                                  | 插入一个点                           |
| `on(type: "change" \| "error" \| "propertychange" \| "change:active" \| "modifystart" \| "modifyend", callback: (e: OMapModifyEvent) => void): string`   | —                                    |
| `once(type: "change" \| "error" \| "propertychange" \| "change:active" \| "modifystart" \| "modifyend", callback: (e: OMapModifyEvent) => void): string` | —                                    |
| `removePoint(coordinates: OMapCoordinateType): boolean`                                                                                                  | 删除一个点                           |
| `revoke(step?: number): boolean`                                                                                                                         | 撤销修改                             |
| `un(id: string): void`                                                                                                                                   | —                                    |

## 继承成员

继承链：Modify → Interaction
...
- 需要先有承载要素的矢量图层；修改会直接作用于要素的几何。
- 修改完成通常通过事件回调获取更新后的要素。
