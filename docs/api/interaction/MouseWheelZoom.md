...
```ts
import { MouseWheelZoom } from 'openlayers-map-kit'
```

源码：`src/module/interaction/MouseWheelZoom/index.ts`

## 构造

```ts
new MouseWheelZoom(params?: (ManualOmit<Options, ""> & OMapInteractionCommonParamsType))
```
...
| 属性                                        | 说明                                                 |
| ------------------------------------------- | ---------------------------------------------------- |
| `events: Event<OMapMouseWheelZoomEventMap>` | 收窄交互事件总线类型（构造器中以具体事件映射实例化） |

## 方法

| 方法                                                                                                                             | 说明 |
| -------------------------------------------------------------------------------------------------------------------------------- | ---- |
| `on(type: "change" \| "error" \| "propertychange" \| "change:active", callback: (e: OMapMouseWheelZoomEvent) => void): string`   | —    |
| `once(type: "change" \| "error" \| "propertychange" \| "change:active", callback: (e: OMapMouseWheelZoomEvent) => void): string` | —    |
| `un(id: string): void`                                                                                                           | —    |

## 继承成员

继承链：MouseWheelZoom → Interaction
...
- 通常已包含在地图默认交互中；可通过选项调整缩放灵敏度与动画时长。
