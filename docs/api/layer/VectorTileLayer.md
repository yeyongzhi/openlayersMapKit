...
```ts
import { VectorTileLayer } from 'openlayers-map-kit'
```

源码：`src/module/layer/VectorTileLayer/index.ts`

## 构造

```ts
new VectorTileLayer(options?: OMapVectorTileLayerParamsType<P>)
```
...
| 方法                                                           | 说明                                     |
| -------------------------------------------------------------- | ---------------------------------------- |
| `getVectorTileSource(): VectorTileSource<FeatureLike> \| null` | 获取图层关联的 OMap 矢量瓦片数据源包装。 |

## 继承成员

继承链：VectorTileLayer → BaseLayer
