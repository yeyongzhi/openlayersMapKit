...
WMTS 图层，加载 OGC Web Map Tile Service 提供的瓦片。
...
```ts
new WMTSLayer(options?: OMapWMTSLayerParamsType<P>)
```
...
| 方法                                  | 说明                                  |
| ------------------------------------- | ------------------------------------- |
| `getWMTSSource(): WMTSSource \| null` | 获取图层关联的 OMap WMTS 数据源包装。 |

## 继承成员

继承链：WMTSLayer → BaseLayer
...
- `tileGrid` 使用本库自定义的网格描述（`resolutions`、`matrixIds`、`sizes`），**不是** OpenLayers 的 `WMTSTileGrid` 实例。
- `source` 为必填项。
- 矩阵集标识（`matrixSet`）与比例尺层级需与服务端能力文档严格对应，否则会出现空白或错位。
