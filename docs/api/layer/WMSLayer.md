...
WMS 图层，加载 OGC Web Map Service 提供的地图图片。
...
```ts
new WMSLayer(options?: OMapWMSLayerParamsType<P>)
```
...
| 方法                                    | 说明                                     |
| --------------------------------------- | ---------------------------------------- |
| `getWMSSource(): TileWMSSource \| null` | 获取图层关联的 OMap TileWMS 数据源包装。 |

## 继承成员

继承链：WMSLayer → BaseLayer
...
- 需要服务地址与图层名等 WMS 参数，具体字段以服务端能力文档为准。
- `source` 为必填项。
