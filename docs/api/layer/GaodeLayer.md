...
```ts
new GaodeLayer(type: GaodeLayerTypeEnum, options: OMapGaodeLayerParamsType)
```
...
| 属性                            | 说明     |
| ------------------------------- | -------- |
| `gaodeType: GaodeLayerTypeEnum` | 图层类型 |

## 继承成员

继承链：GaodeLayer → TileLayer → BaseLayer
...
- 底图类型见 [`GaodeLayerType`](./GaodeLayerType.md)。
- 内置地址为高德公开瓦片服务，商用前请确认其服务条款与配额要求。
