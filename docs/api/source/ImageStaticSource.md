...
```ts
import { ImageStaticSource } from 'openlayers-map-kit'
```

源码：`src/module/source/ImageStaticSource/index.ts`

## 构造

```ts
new ImageStaticSource(params: OMapImageStaticSourceParamsType)
```
...
| 方法                       | 说明                                   |
| -------------------------- | -------------------------------------- |
| `getImageExtent(): Extent` | 获取图片所覆盖的地理范围               |
| `getSource(): Static`      | 获取原生 OpenLayers 静态图片数据源实例 |
| `getUrl(): string`         | 获取图片地址                           |

## 继承成员

继承链：ImageStaticSource → ImageSource → Source
