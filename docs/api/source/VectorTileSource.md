...
```ts
new VectorTileSource(params: OMapVectorTileSourceParamsType<FeatureType>)
```
...
继承链：VectorTileSource → TileSource → Source
...
- `getOverlaps()` / `setOverlaps()` 控制是否允许要素重叠，影响渲染与命中表现。
- 矢量瓦片中的要素多为渲染要素（RenderFeature），读取属性前请先确认其形态。
