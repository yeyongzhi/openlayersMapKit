...
调试瓦片数据源，在瓦片上绘制编号与边界等调试信息。

## 引入

```ts
import { TileDebugSource } from 'openlayers-map-kit'
```

源码：`src/module/source/TileSource/subClass/TileDebugSource/index.ts`

## 构造

```ts
new TileDebugSource(params?: OMapTileDebugSourceParamsType)
```

## 继承成员

继承链：TileDebugSource → TileSource → Source
