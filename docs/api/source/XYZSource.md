...
```ts
new XYZSource(params?: OMapXYZSourceParamsType)
```

## 继承成员

继承链：XYZSource → TileSource → Source
...
- URL 模板中的 `{z}`、`{x}`、`{y}` 占位符由数据源自动替换。
- 需要负载均衡时可传入多域名模板（如 `{a-c}` 形式），具体以服务端约定为准。

## 相关

- 基类：[TileSource](./TileSource.md)
- 承载图层：[XYZLayer](../layer/XYZLayer.md)
