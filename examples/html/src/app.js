;(function () {
  ...
  if (!window.OMap) throw new Error('OMap UMD 资源加载失败')
  ...
  const mapElement = required('#map')
  const tabsElement = required('#case-tabs')
  const titleElement = required('#case-title')
  const kickerElement = required('#case-kicker')
  const descriptionElement = required('#case-description')
  const actionsElement = required('#case-actions')
  const statusElement = required('#status')
  const codeElement = required('#case-code')
  let map

  const setStatus = (message) => (statusElement.textContent = message)
  const createGaodeLayer = (type = GaodeLayerType.Vec) =>
    new GaodeLayer(type, {
      preload: 0,
  ...
  const createMap = () =>
    new OMapMap(mapElement, {
      view: { center: project(BEIJING_CENTER), zoom: 11 }
    })
  const disposeCurrentMap = () => {
  ...
  const cases = [
    {
      ...
      description: '创建最小地图实例并加载高德矢量底图，验证 UMD 全局入口和基础视图。',
      code: `const center = OMap.ProjUtil.fromLonLat([116.397428, 39.90923])
const map = new OMap.Map('#map', {
  view: { center, zoom: 11 }
})
map.addLayer(createGaodeLayer(OMap.GaodeLayerType.Vec))`,
      mount() {
        map = createMap()
    ...
    },
    {
      ...
      code: `const layer = new OMap.VectorLayer({ id: 'geometry-demo' })
layer.addFeatures([
  new OMap.Point([116.397, 39.909]),
  new OMap.LineString([[116.32, 39.86], [116.49, 39.95]]),
  new OMap.Polygon([[[116.32, 39.85], ...]])
])
map.addLayer(layer)`,
      mount() {
        map = createMap()
...
activeLayer.dispose()
activeLayer = createGaodeLayer(OMap.GaodeLayerType.Img)
map.addLayer(activeLayer)`,
      mount() {
        map = createMap()
        let activeLayer = createGaodeLayer()
        map.addLayer(activeLayer)
        const switchLayer = (type, label) => {
    ...
    },
    {
      ...
      code: `map.addControl(new OMap.Zoom('demo-zoom'))
map.addControl(new OMap.FullScreen('demo-fullscreen'))
map.zoomIn()
map.zoomOut()`,
      mount() {
        map = createMap()
        ...
        addAction('放大', () => map && map.zoomIn(), true)
        addAction('缩小', () => map && map.zoomOut())
...
map = new OMap.Map(element, options)
map.addLayer(createGaodeLayer())`,
      mount() {
        const recreate = () => {
          disposeCurrentMap()
