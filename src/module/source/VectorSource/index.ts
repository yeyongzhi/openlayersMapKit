...
export default class VectorSource extends Source<OMapVectorSourceType> {
  ...
  removeFeatures(features: Array<OMapVectorSourceFeature>) {
    ...
    this._source.removeFeatures(
      features.map((feature) => this.getOlFeature(feature, 'removeFeatures'))
    )
  ...
  protected getOlFeature(
    feature: OMapVectorSourceFeature,
    methodName: string
  ): OMapVectorSourceOlFeature {
  ...
  protected validateCallback(callback: (...args: never[]) => unknown, methodName: string) {
