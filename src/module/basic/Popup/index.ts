...
export default class Popup {
  ...
  constructor(params: OMapPopupParamsType) {
    ...
    if (
      isDefined(_params.content) &&
      isString(_params.content) &&
      !isDefined(_params.element)
    ) {
    ...
    this._popup = new OlOverlay({
      ..._params,
      offset: isDefined(_params.offset)
        ? handleGetPixelValue(_params.offset)
        : undefined,
      position: isDefined(_params.position)
        ? handleGetLnglatValue(_params.position)
        : undefined,
    });
