...
export default class Extent {
  ...
  constructor(...args: [number, number, number, number] | [number[]]) {
    ...
    if (args.length === 4) {
      ...
      if (isNumber(minX) && isNumber(minY) && isNumber(maxX) && isNumber(maxY)) {
        ...
        error_(createMessage('constructor', commonMessage.paramsInvalidFormat('extent')))
      }
   ...
   * 以字符串的形式输出边界范围
   *
   * @returns {string} 边界范围（字符串）
   */
  toString(place?: number): string {
    const precision = place ?? 3
    return `[${this._extent[0].toFixed(precision)}, ${this._extent[1].toFixed(precision)}, ${this._extent[2].toFixed(precision)}, ${this._extent[3].toFixed(precision)}]`
  }
  ...
  static boundingExtent(coordinates: Array<OMapCoordinateType>): Extent {
    if (!isDefined(coordinates)) {
      error_(createMessage('boundingExtent', commonMessage.paramsNotDefined('coordinates')))
    }
    if (!isArray(coordinates)) {
      error_(createMessage('boundingExtent', '参数coordinates格式错误，必须为数组'))
    }
    const validList = coordinates.filter((c) => {
      return isValidCoordinate(c)
    })
    if (validList.length < coordinates.length) {
      warn_(createMessage('boundingExtent', commonMessage.haveInvalidDataItem('coordinates')))
    }
    const positions = validList.map((c) => {
      return handleGetLngLatValue(c)
    })
    const extentValue = OlExtent.boundingExtent(positions)
   ...
   */
  static containsCoordinate(extent: OMapExtentType, coordinate: OMapCoordinateType): boolean {
    if (!isDefined(extent) || !isDefined(coordinate)) {
      error_(
        createMessage('containsCoordinate', commonMessage.paramsNotDefined('extent or coordinate'))
      )
    }
    const extentValue = handleGetExtentValue(extent)
    const coordinateValue = handleGetLngLatValue(coordinate)
    return OlExtent.containsCoordinate(extentValue, coordinateValue)
  }
   ...
   * @returns 判断结果
   */
  static containsExtent(extent1: OMapExtentType, extent2: OMapExtentType): boolean {
    if (!isDefined(extent1) || !isDefined(extent2)) {
      error_(createMessage('containsExtent', commonMessage.paramsNotDefined('extent1 or extent2')))
    }
    const firstExtent = handleGetExtentValue(extent1)
    const secondExtent = handleGetExtentValue(extent2)
    return OlExtent.containsExtent(firstExtent, secondExtent)
  }
  ...
  static containsXY(extent: OMapExtentType, x: number, y: number): boolean {
    ...
    const extentValue = handleGetExtentValue(extent)
    return OlExtent.containsXY(extentValue, x, y)
  }
  ...
  static equals(extent1: OMapExtentType, extent2: OMapExtentType): boolean {
    if (!isDefined(extent1) || !isDefined(extent2)) {
      error_(createMessage('equals', commonMessage.paramsNotDefined('extent1 or extent2')))
    }
    const firstExtent = handleGetExtentValue(extent1)
    const secondExtent = handleGetExtentValue(extent2)
    return OlExtent.equals(firstExtent, secondExtent)
  }

  static extend(extent1: OMapExtentType, extent2: OMapExtentType): Extent {
    if (!isDefined(extent1) || !isDefined(extent2)) {
      error_(createMessage('extend', commonMessage.paramsNotDefined('extent1 or extent2')))
    }
    const firstExtent = handleGetExtentValue(extent1)
    const secondExtent = handleGetExtentValue(extent2)
    return new Extent(OlExtent.extend(firstExtent, secondExtent))
  }

  static getArea(extent: OMapExtentType): number {
    if (!isDefined(extent)) {
      error_(createMessage('getArea', commonMessage.paramsNotDefined('extent')))
    }
    const extentValue = handleGetExtentValue(extent)
    return OlExtent.getArea(extentValue)
  }
  ...
  static intersects(extent1: OMapExtentType, extent2: OMapExtentType): boolean {
    if (!isDefined(extent1) || !isDefined(extent2)) {
      error_(createMessage('intersects', commonMessage.paramsNotDefined('extent1 or extent2')))
    }
    const firstExtent = handleGetExtentValue(extent1)
    const secondExtent = handleGetExtentValue(extent2)
    return OlExtent.intersects(firstExtent, secondExtent)
  }

  static isEmpty(extent: OMapExtentType): boolean {
    if (!isDefined(extent)) {
      error_(createMessage('isEmpty', commonMessage.paramsNotDefined('extent')))
    }
    const extentValue = handleGetExtentValue(extent)
    return OlExtent.isEmpty(extentValue)
  }
}
