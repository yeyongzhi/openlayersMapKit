...
function readFeatures(
  ...
  const features = (format as OMapKMLFormatInstanceType).readFeatures(
    source,
    defaultValue(options, {})
  )
...
function writeFeatures(
  ...
  const source = (format as OMapKMLFormatInstanceType).writeFeatures(
    features.map(
...
function writeFeaturesNode(
  format: OMapKMLFormatInstanceType,
  features: Array<BasicFeature<OlGeometryType>>,
  options?: OMapFormatWriteFeatureOptionsType
) {
  const source = (format as OMapKMLFormatInstanceType).writeFeaturesNode(
    features.map(
      (feature: BasicFeature<OlGeometryType>) => feature.getFeature() as OlFeatureInstanceType
    ),
    Object.assign({}, DEFAULT_FORMAT_WRITE_FEATURE_OPTIONS, defaultValue(options, {}))
