 ...
 * 交互基类。
 *
 * @template T - 原生 OpenLayers Interaction 类型。
 * @template P - 交互属性字典。默认 {@link PropertiesType}；
  ...
  implements Disposable, Removable
{
  ...
  protected subscribeEvent(
    ...
    this.assertActive(once ? 'once' : 'on')
  ...
  setId(id: OMapInteractionIdType) {
    ...
    if (!isDefined(id)) {
      warn_(createMessage('initInteractionId', commonMessage.paramsNotDefined('id')))
      return
    }
    this.id = id
  }
  ...
  setActive(active: boolean) {
    this.assertActive('setActive')
    if (!isBoolean(active)) {
      warn_(createMessage('setActive', commonMessage.paramsInvalidFormat('active', 'boolean')))
      return
    }
   ...
   */
  getInteraction(): T {
    this.assertActive('getInteraction')
