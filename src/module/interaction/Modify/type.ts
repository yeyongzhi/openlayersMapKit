...
export type OlModifyParamsType = ConstructorParameters<
  typeof OlInteraction.Modify
>[0];
...
export type OMapInteractionModifyEventType =
  (typeof OMapInteractionModifyEventTypes)[number];
...
/** 事件名 → 用户回调参数映射 */
export type OMapModifyEventMap = Record<
...
export function isOMapInteractionModifyEventType(
  ...
  return (
    isString(value) &&
    OMapInteractionModifyEventTypes.includes(
      value as OMapInteractionModifyEventType,
    )
  );
}
