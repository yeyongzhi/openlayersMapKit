...
class Extent {
  ...
  static containsCoordinate(extent, coordinate) {
    if (!isDefined(extent) || !isDefined(coordinate)) return void 0;
...
class MultiPoint extends BasicFeature {
  ...
  appendPoint(pointOrpointCoordinates) {
    ...
    if (pointOrpointCoordinates instanceof Point) {
      ...
      _point = new OlGeometry.Point(pointOrpointCoordinates.toArray());
    } else if (isCoordinatesType(pointOrpointCoordinates)) {
      _point = new OlGeometry.Point(pointOrpointCoordinates);
    }
    if (!isDefined(_point)) {
  ...
  revoke(step = 1) {
    ...
    features.forEach((f) => {
      let target = this.layer.getFeatures().find((item) => {
        ...
        return OlUtil.getUid(item._feature) === f.originFeatureId;
      });
      if (isDefined(target)) {
...
function getTargetFeature(id) {
  ...
  if (selectLayers.length) {
    for (const layer of selectLayers) {
      let target = layer.getFeatures().find((f) => OlUtil.getUid(f._feature) === id);
      if (target) {
        result = target;
      }
    }
  ...
  initFilter(filter) {
    if (isDefined(filter)) {
      return (feature, layer) => {
        ...
        let targetLayer = (_a = this.map) == null ? void 0 : _a.getAllLayers().find((l) => OlUtil.getUid(l._layer) === OlUtil.getUid(layer));
  ...
  initSelectEvent() {
    ...
    this._interaction.on("select", (e) => {
      ...
      this.selected = selected.map((s) => {
        return getTargetFeature(OlUtil.getUid(s));
      }).filter((f) => f !== null);
      this.deselected = deselected.map((d) => {
        return getTargetFeature(OlUtil.getUid(d));
      }).filter((f) => f !== null);
    });
  }
  getSelected() {
