...
class DoubleClickZoom extends Interaction {
  constructor(params) {
    ...
    this._interaction = new OlInteraction.DoubleClickZoom(Object.assign(OMAP_INTERACTION_DEFAULT_PARAMS, defaultDoubleClickZoomOptions, defaultValue(params, {})));
    this.initInteractionEvent();
  }
}
  ...
  revoke(step = 1) {
    ...
    features.forEach((f) => {
      let target = this.layer.getFeatures().find((item) => {
        ...
        return OlUtil.getUid(item.getFeature()) === f.originFeatureId;
      });
      if (isDefined(target)) {
